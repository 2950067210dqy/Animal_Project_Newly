...
class UFC_gas_path_system_run_thread(MyQThread):
        ...
        if mouse_cages_inc is not None and len(mouse_cages_inc) > 0:
            ...
            AsyPromise(self.send_thread.Send).then(
                ...
                lambda r: AsyPromise(self.close_last_mouse_cage_gas, port=port, mouse_cages_inc=mouse_cages_inc).then(
                    lambda r: AsyPromise(self.finsh_one_batch, port=port, mouse_cages_inc=mouse_cages_inc).then(
    ...
    def close_last_mouse_cage_gas(self, resolve, reject, port, mouse_cages_inc):
        ...
        """
        time.sleep(0.01)
        mouse_cage_number_addr_single = mouse_cages_inc[
            self.mouse_cage_index - 1 if self.mouse_cage_index != 0 else len(mouse_cages_inc) - 1]
...
class UFC_gas_path_system(Gas_path_system):
    ...
    def start(self, resolve, reject):
        ...
        time.sleep(0.01)
        self.update_status_main_signal_gui_update.send(
            f"{time_util.get_format_from_time(time.time())} | UFC 开始启动{'.' * 100}")
    ...
    def run(self, resolve, reject):
        ...
        time.sleep(0.01)
        self.update_status_main_signal_gui_update.send(
            f"{time_util.get_format_from_time(time.time())} | UFC 开始运行{'.' * 100}")
    ...
    def stop(self, resolve, reject):
        ...
        time.sleep(0.01)
        self.update_status_main_signal_gui_update.send(
            f"{time_util.get_format_from_time(time.time())} | UFC 正在停止{'.' * 100}")
...
class UGC_gas_path_system(Gas_path_system):
    ...
    def start(self, resolve, reject):
        ...
        time.sleep(0.01)
        self.update_status_main_signal_gui_update.send(
            f"{time_util.get_format_from_time(time.time())} | UGC 正在启动{'.' * 100}")
    ...
    def stop(self, resolve, reject):
        ...
        """
        self.update_status_main_signal_gui_update.send(
            f"{time_util.get_format_from_time(time.time())} | UGC 正在停止{'.' * 100}")
...
class ZOS_gas_path_system(Gas_path_system):
    ...
    def start(self, resolve, reject):
        ...
        time.sleep(0.01)
        self.update_status_main_signal_gui_update.send(
            f"{time_util.get_format_from_time(time.time())} | ZOS 正在启动{'.' * 100}")
        # 1.读取系统状态
        port = global_setting.get_setting("port", None)
        if port is None:
            self.update_status_main_signal_gui_update.send(
    ...
    def run(self, resolve, reject):
        ...
        time.sleep(0.01)
        self.update_status_main_signal_gui_update.send(
            f"{time_util.get_format_from_time(time.time())} | ZOS 开始运行{'.' * 100}")
        # 1.循环读取氧浓度
    ...
    def stop(self, resolve, reject):
        ...
        """
        self.update_status_main_signal_gui_update.send(
            f"{time_util.get_format_from_time(time.time())} | ZOS 正在停止{'.' * 100}")
