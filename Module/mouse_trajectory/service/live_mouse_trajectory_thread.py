...
class MouseTrajectoryThread(MyQThread):
    ...
    def __init__(self):
        ...
        self.output_flush_interval_seconds = 30.0
        self.plot_window_seconds = 600.0
    ...
    def submit_frame(self, cage_number: int, frame_payload: dict[str, Any]):
        if not self.accepting_frames or not frame_payload or frame_payload.get("frame") is None:
            return
        cage_number = int(cage_number)
        frame_id = int(frame_payload.get("frame_id", 0) or 0)
        frame_key = (cage_number, frame_id)
        with self.pending_lock:
            if frame_id > 0 and frame_key in self.pending_frame_keys:
                return
    ...
    def _pop_pending_frames(self, max_items: int) -> list[tuple[int, dict[str, Any]]]:
        ...
        with self.pending_lock:
            while self.pending_frames and len(frame_items) < max_items:
                cage_number, frame_payload = self.pending_frames.popleft()
                frame_id = int(frame_payload.get("frame_id", 0) or 0)
                if frame_id > 0:
                    self.pending_frame_keys.discard((int(cage_number), frame_id))
                frame_items.append((int(cage_number), frame_payload))
        return frame_items
    ...
    def _process_frame(self, cage_number: int, frame_payload: dict[str, Any]):
        ...
        self.processed_frame_count_by_cage[cage_number] = self.processed_frame_count_by_cage.get(cage_number, 0) + 1
        ...
        self.trajectory_ready.emit(
            {
                ...
                "plot_title": self.latest_plot_title_by_cage.get(cage_number, self._build_processing_title(cage_number, window_start, window_end)),
    ...
    def _process_frame_batch(self, frame_items: list[tuple[int, dict[str, Any]]]):
        ...
        if mouse_sources:
            try:
                ...
                logger.error(f"mouse trajectory batch predict failed count={len(mouse_sources)}: {error}")
                for source_index in mouse_source_indexes:
                    prepared_items[source_index]["status"] = "error"
                    prepared_items[source_index]["error"] = str(error)
    ...
    def _maybe_flush_outputs(
        ...
        if current_window_index > last_plotted_window:
            for window_index in range(last_plotted_window + 1, current_window_index):
                window_plot_paths = self._save_window_plot(
                    cage_number=cage_number,
                    image_file=image_file,
                    rows=rows,
                    shift_log=shift_log,
                ...
                if window_plot_paths:
                    latest_plot_paths = window_plot_paths
