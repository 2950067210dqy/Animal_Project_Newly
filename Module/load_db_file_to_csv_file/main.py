...
class Main_load_db_file_to_csv_file_service(BaseService):
    # 组件服务
    def __init__(self):
        super().__init__()
        pass
    def start(self,resolve,reject):
        # 打开实验设置文件
        file_path, _ = QFileDialog.getOpenFileName(self.module.main_gui, "选择db文件", "", f"Mdata files (*.{custom_template_file_util.db_extension_name});")
        if file_path:
            custom_data_file_util.export_data_to_csv(export_file_path=None,
                                                     file_name= os.path.splitext(os.path.basename(file_path))[0],file_path=file_path)
...
class Main_load_db_file_to_csv_file_widget(BaseInterfaceWidget):
    # 组件自定义界面
    def __init__(self):
        super().__init__()
...
class Main_load_db_file_to_csv_file_Module(BaseModule):
    def __init__(self):
        super().__init__()
    ...
    def get_name(self):
        """返回组件名称"""
        return "Main_load_db_file_to_csv_file"
        pass
    def get_title(self):
        """获取组件title"""
    ...
    def create_service(self) -> BaseService:
        """创建并返回组件的相关服务"""
        service = Main_load_db_file_to_csv_file_service()
    ...
    def get_interface_widget(self) -> BaseInterfaceWidget:
        """返回自定义界面构建器"""
        widget_builder =Main_load_db_file_to_csv_file_widget()
